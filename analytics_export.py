...
def cached_positions(limit: int = 5000) -> list[dict[str, Any]]:
    ...
    ensure_alpha_tables(); conn = get_conn()
    rows = conn.execute("""SELECT wallet,market,title,outcome,size,value,avg_price,current_price,token_id,condition_id,last_seen
                         FROM wallet_positions ORDER BY last_seen DESC LIMIT ?""", (limit,)).fetchall()
    conn.close()
    keys = ["wallet","market","title","outcome","size","value","avg_price","current_price","token_id","condition_id","last_seen"]
    return [dict(zip(keys, r)) for r in rows]


def clear_old_consensus(keep_last: int = 300) -> None:
    """Keep the consensus table small and fast."""
    ensure_alpha_tables(); conn = get_conn()
    conn.execute("DELETE FROM consensus_signals WHERE id NOT IN (SELECT id FROM consensus_signals ORDER BY id DESC LIMIT ?)", (keep_last,))
    conn.commit(); conn.close()

def save_discovered_wallet(wallet: str, source: str = "leaderboard", category: str = "OVERALL", time_period: str = "MONTH", order_by: str = "PNL", pnl: float = 0.0, volume: float = 0.0, rank: int = 0, raw: Any = None, label: str = "") -> None:
    """Store a discovered wallet and add it to alpha_wallets without destroying labels."""
    ensure_alpha_tables()
    w = wallet.lower().strip()
    ...
    conn.execute(
        """INSERT INTO alpha_wallet_discovery(wallet,source,category,time_period,order_by,pnl,volume,rank,raw_json,first_seen,last_seen)
           VALUES(?,?,?,?,?,?,?,?,?,?,?)
           ON CONFLICT(wallet) DO UPDATE SET source=excluded.source, category=excluded.category, time_period=excluded.time_period,
           order_by=excluded.order_by, pnl=excluded.pnl, volume=excluded.volume, rank=excluded.rank, raw_json=excluded.raw_json, last_seen=excluded.last_seen""",
...
def save_alpha_scan_run(source: str, category: str, time_period: str, order_by: str, wallets_found: int, wallets_added: int, wallets_scored: int, top_wallet: str = "", top_score: float = 0.0, status: str = "ok", error: str = "") -> None:
    ensure_alpha_tables(); conn = get_conn()
    conn.execute(
        """INSERT INTO alpha_scan_runs(source,category,time_period,order_by,wallets_found,wallets_added,wallets_scored,top_wallet,top_score,status,error,created_at)
           VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
        (source, category, time_period, order_by, wallets_found, wallets_added, wallets_scored, top_wallet, top_score, status, error[:1000], now_iso()),
    )
    conn.commit(); conn.close()
...
def latest_alpha_scans(limit: int = 5) -> list[dict[str, Any]]:
    ensure_alpha_tables(); conn = get_conn()
    rows = conn.execute("""SELECT source,category,time_period,order_by,wallets_found,wallets_added,wallets_scored,top_wallet,top_score,status,error,created_at
                         FROM alpha_scan_runs ORDER BY id DESC LIMIT ?""", (limit,)).fetchall()
    conn.close()
    keys = ["source","category","time_period","order_by","wallets_found","wallets_added","wallets_scored","top_wallet","top_score","status","error","created_at"]
    return [dict(zip(keys, r)) for r in rows]
