...
from bot.alpha_store import (
    add_alpha_wallet, ensure_alpha_tables, get_alpha_setting, latest_consensus,
...
def alpha_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🖥 Terminal", callback_data="alpha_terminal"), InlineKeyboardButton("🔥 Consensus", callback_data="alpha_consensus")],
...
async def alpha_start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Tracked smart wallets: <b>{count}</b>\n\n"
...
async def topwallets_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    wallets = _wallets()
    if not wallets:
        await _send_or_edit(update, "🏆 <b>Top Wallets</b>\n\nAdd wallets first with /alpha_addwallet 0xWallet label", alpha_menu())
        return
    if update.message:
...
async def terminal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = ["🖥 <b>POLYALPHA TERMINAL</b>", "<code>Bloomberg-style Polymarket intelligence</code>", f"Smart wallets: <b>{len(wallets)}</b>"]
