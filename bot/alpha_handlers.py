...
def _fresh_hint() -> str:
    return "\n\n<code>/scan_wallets OVERALL MONTH PNL 50</code> refreshes data. Terminal/TopWallets use cache for speed."
...
async def scan_wallets_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Limit: <b>{limit}</b>\n\n"
        "Fast mode: leaderboard scores first, positions only for top wallets.",
        parse_mode="HTML",
    )
...
async def topwallets_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Fast UI: use saved scores from the latest leaderboard scan instead of rescanning every wallet.
    saved = top_saved_wallet_scores(20)
    if not saved:
        await _send_or_edit(update, "🏆 <b>Top Wallets</b>\n\nNo scored wallets yet. Run:\n<code>/scan_wallets OVERALL MONTH PNL 50</code>", alpha_menu())
        return
    lines = ["🏆 <b>Top Smart Wallets</b>", "<code>cached from latest leaderboard scan / click wallets for Polymarket profile</code>"]
    for i, x in enumerate(saved[:15], 1):
        w = x.get("wallet") or ""
        lines.append(
            f"\n{i}. {_wallet_link(w)} — <b>{(x.get('score') or 0):.1f}/100</b>\n"
            f"PnL: {_money(x.get('pnl') or 0)} | Vol: {_money(x.get('volume') or 0)} | Open: {_money(x.get('open_value') or 0)}\n"
            f"ROI: {_pct(x.get('roi') or 0)} | Win: {_pct(x.get('winrate') or 0)} | Trades: {int(x.get('trades') or 0)}\n"
            f"Copy: <code>{_esc(w)}</code>"
        )
    lines.append(_fresh_hint())
    await _send_or_edit(update, "\n".join(lines), alpha_menu())


async def consensus_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Low latency default: show saved consensus. Use /consensus_refresh for a live rebuild.
    cached = latest_consensus(10)
    if not cached:
        await _send_or_edit(update, "🔥 <b>Consensus</b>\n\nNo consensus cache yet. Run:\n<code>/scan_wallets OVERALL MONTH PNL 50</code>", alpha_menu())
        return
    lines = ["🔥 <b>Smart Money Consensus</b>", "<code>cached high-overlap positions from top wallets</code>"]
    for i, srow in enumerate(cached[:10], 1):
        market = srow.get("market") or ""
        lines.append(
            f"\n{i}. <b>{_esc(srow.get('title'))[:90]}</b>\n"
            f"Outcome: <b>{_esc(srow.get('outcome'))}</b> | Signal: <b>{(srow.get('score') or 0):.1f}/100</b> | {_esc(srow.get('confidence'))}\n"
            f"Wallets: {srow.get('wallets')} | Value: {_money(srow.get('total_value') or 0)} | Avg px: {(srow.get('avg_price') or 0):.3f}\n"
            f"Fair: {(srow.get('fair_value') or 0):.3f} | Edge: {(srow.get('edge') or 0):+.3f}\n"
            f"Market: <code>{_esc(market)}</code>"
        )
    lines.append("\nUse <code>/consensus_refresh</code> only when you want a slower live rebuild.")
    await _send_or_edit(update, "\n".join(lines), alpha_menu())
...
async def consensus_refresh_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    wallets = _wallets()[:40]
    if not wallets:
        await _send_or_edit(update, "No smart wallets saved. Run /scan_wallets first.", alpha_menu())
        return
    if update.message:
        await update.message.reply_text("Refreshing consensus from top wallets…")
    signals = await SmartMoneyEngine(wallets).consensus(min_wallets=2, top_n=10)
    if not signals:
        await _send_or_edit(update, "No live consensus found. Try scanning more wallets.", alpha_menu())
        return
    lines = ["🔥 <b>Live Consensus Refresh</b>"]
    for i, sig in enumerate(signals[:10], 1):
        lines.append(
            f"\n{i}. <b>{_esc(sig.title)[:90]}</b>\n"
            f"{_esc(sig.outcome)} | Signal {sig.score}/100 | {sig.wallets} wallets | Value {_money(sig.total_value)}\n"
            f"Avg {sig.avg_price} | Fair {sig.fair_value} | Edge {sig.edge:+.3f}"
        )
    await _send_or_edit(update, "\n".join(lines), alpha_menu())
...
async def terminal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cached:
        t = cached[0]
        lines += ["", "🚀 <b>Top Signal</b>", f"<b>{_esc(t.get('title'))[:95]}</b>", f"Outcome: <b>{_esc(t.get('outcome'))}</b>", f"Consensus: <b>{(t.get('score') or 0):.1f}/100</b> | {_esc(t.get('confidence'))}", f"Wallets: {t.get('wallets')} | Value: {_money(t.get('total_value') or 0)}", f"Avg price: {(t.get('avg_price') or 0):.3f} | Fair: {(t.get('fair_value') or 0):.3f} | Edge: {(t.get('edge') or 0):+.3f}"]
    else:
        lines.append("\nNo consensus cache yet. Run /scan_wallets.")
    if scores:
        lines.append("\n🏆 <b>Top Wallet Cache</b>")
        for s in scores[:3]: lines.append(f"• {_wallet_link(s['wallet'])} — {s['score']}/100")
