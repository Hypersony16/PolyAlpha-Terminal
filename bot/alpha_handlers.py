...
def alpha_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🖥 Terminal", callback_data="alpha_terminal"),
         InlineKeyboardButton("🚀 Actionable", callback_data="alpha_actionable")],
...
async def actionable_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rows = actionable_signals(limit=8, min_alpha=55)
    lines = [
        "🚀 <b>ACTIONABLE ALPHA</b>",
        "<code>positive edge only · cached · decision-first</code>",
        _divider(),
    ]
    if not rows:
        lines += ["", "No clean actionable signals right now.", "Run /scan_wallets or /alpha_scan_all, then try again."]
    for i, r in enumerate(rows, 1):
        alpha = r.get("alpha_score") or 0
        edge = r.get("edge") or 0
        comps = r.get("alpha_components") or {}
        decision = "BUY WATCH" if alpha >= 75 else "WATCH" if alpha >= 60 else "LOW PRIORITY"
        lines.append(
            f"\n<b>{i}. {_alpha_tier(alpha)} — {_esc(r.get('title','')[:78])}</b>\n"
            f"Decision: <b>{decision}</b>  ·  Outcome: <b>{_esc(r.get('outcome'))}</b>\n"
            f"Alpha: <b>{alpha:.0f}/100</b>  ·  Edge: <b>{edge:+.3f}</b>  ·  {r.get('wallets')} wallets\n"
            f"Value: <b>{_money(r.get('total_value') or 0)}</b>  ·  Avg smart score: {(r.get('avg_wallet_score') or 0):.0f}\n"
            f"Price <code>{(r.get('avg_price') or 0):.3f}</code> → Fair <code>{(r.get('fair_value') or 0):.3f}</code>\n"
            f"Why: consensus {comps.get('consensus',0):.0f}, quality {comps.get('wallet_quality',0):.0f}, capital {comps.get('capital',0):.0f}, edge {comps.get('edge',0):.0f}\n"
            f"{_market_link(r.get('market',''), '🔗 Open market')}  ·  <code>{_esc(r.get('market',''))}</code>"
        )
    lines.append(f"\n{_divider()}\nUse /research market-slug for deeper view.")
    await _send_or_edit(update, "\n".join(lines), alpha_menu())
