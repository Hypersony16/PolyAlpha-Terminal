...
async def build_btc_model(market: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Previous live paper stats showed raw probabilities were massively overconfident.
    # So we shrink probabilities toward 50% and cap extremes.
    mom_score = (ret_1m * 3.0) + (ret_3m * 1.8) + (ret_5m * 1.0) + (ret_15m * 0.35)
    momentum_adj = (_sigmoid(mom_score * 55.0) - 0.5) * 0.06

    raw_up = _clamp(base_up_prob + momentum_adj, 0.02, 0.98)
    calibrated_up = 0.5 + (raw_up - 0.5) * 0.50
    model_up = _clamp(calibrated_up, 0.30, 0.70)
    model_down = 1.0 - model_up

    ema_fast = _ema(closes[-12:], 5)
    ema_slow = _ema(closes[-30:], 15)
    trend_side = "UP" if ema_fast >= ema_slow else "DOWN"
    last_1m_abs_pct = abs(ret_1m) * 100.0
    spike_flag = last_1m_abs_pct >= 0.18

    # Real Polymarket odds. If missing, mark as fallback and use 50/50 only for display/model.
    ...
    if up_market is None and down_market is None:
        ...
        odds_source = str((market or {}).get("odds_source") or f"inferred/{(market or {}).get('down_price_source','?')}")
    elif down_market is None:
        up_market = _clamp(float(up_market), 0.01, 0.99)
        down_market = 1.0 - up_market
        odds_source = str((market or {}).get("odds_source") or f"{(market or {}).get('up_price_source','?')}/inferred")
    else:
        up_market = _clamp(float(up_market), 0.01, 0.99)
        down_market = _clamp(float(down_market), 0.01, 0.99)
        odds_source = str((market or {}).get("odds_source") or f"{(market or {}).get('up_price_source','?')}/{(market or {}).get('down_price_source','?')}")
        if "missing" in odds_source:
            odds_source = f"{(market or {}).get('up_price_source','?')}/{(market or {}).get('down_price_source','?')}"
    ...
    # Conservative confidence after calibration.
    abs_edge = max(0.0, edge)
    if ev_per_dollar >= 0.08 and abs_edge >= 0.08 and model_prob >= 0.58:
        confidence = "High"
    elif ev_per_dollar >= 0.05 and abs_edge >= 0.05 and model_prob >= 0.56:
        confidence = "Medium"
    else:
        confidence = "Low"
    ...
    return {
        ...
        "last_1m_abs_pct": last_1m_abs_pct,
