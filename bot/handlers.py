...
async def build_market_text(user_id: int):
    ...
    return (
        ...
        f"BTC now: <b>{format_btc_price(model['price'])}</b>\n"
        f"Target/price to beat: <b>{format_btc_price(float(model.get('target_price') or 0)) if model.get('target_price') else 'n/a'}</b>\n"
        f"Time left: <b>{model['window']['left_label']}</b>\n\n"
        f"🟢 UP price: <b>{float(market.get('up_price') or 0):.3f}</b>\n"
        f"🔴 DOWN price: <b>{float(market.get('down_price') or 0):.3f}</b>\n"
...
async def build_accuracy_text(user_id: int):
    ...
    ps = paper_auto_summary(user_id)
    calib = ps.get("calibration", {})
    lines.extend([
        "",
        "<b>Auto paper stats</b>",
        f"Closed trades: {ps['closed']}",
        f"Win rate: {ps['win_rate'] * 100:.1f}%",
        f"PnL: ${ps['pnl']:.2f}",
    ])

    if calib and calib.get("total", 0):
        lines.extend([
            "",
            "<b>Calibration curve</b>",
            f"Resolved: {calib['correct']}/{calib['total']} ({calib['accuracy']*100:.1f}%)",
            f"Avg model: {calib['avg_model']*100:.1f}%",
            f"Avg market: {calib['avg_market']*100:.1f}%",
        ])
        for b in calib.get("buckets", [])[:5]:
            lines.append(f"• {b['range']}: predicted {b['avg_model']*100:.1f}% / actual {b['hit']*100:.1f}% / n={b['n']}")
...
def build_paper_auto_text(user_id: int):
    ...
    calib = s.get("calibration", {})

    lines = [
        header("🤖 AUTO PAPER TRADING", "real-odds paper mode / BTC 15m"),
        f"🕒 {timestamp_with_seconds()}",
        "",
        f"Status: <b>{status}</b>",
        f"Real odds only: <b>{'ON' if s.get('real_odds_only') else 'OFF'}</b>",
        ...
        f"Strategy mode: <b>{get_strategy_mode(user_id)}</b>",
        f"Last check: <code>{get_last_skip_reason(user_id)}</code>",
        f"Resolution wait: <code>{get_user_setting(user_id, 'paper_resolution_wait', 'none')}</code>",
        "",
        "<b>Rules</b>",
        "• Real Polymarket odds required",
        "• Official Polymarket outcome only",
        "• Max 1 trade per 15m market",
        "• Entry only in Prime phase",
        "• Min edge 12%",
        "• Min EV 7%",
        "• Max entry price 60¢",
        "• Min entry price 35¢",
        "• No entries in last 5 minutes",
        "• Must align with market bias",
        "• Trend + volatility filters ON",
        "• Official result only, instant polling after expiry",
        "",
        "Use /strategy_stats for scalp vs resolution + buckets.",
    ]
    ...
    if calib and calib.get("total", 0):
        lines += [
            ...
            f"Avg model: {calib['avg_model']*100:.1f}%",
            f"Avg market: {calib['avg_market']*100:.1f}%",
            f"Avg edge: {calib['avg_edge']*100:.1f} pts",
            f"Calib PnL: ${calib['pnl']:.2f}",
        ]
        if calib.get("buckets"):
            lines.append("<b>Curve</b>")
            for b in calib["buckets"][:5]:
                lines.append(f"• {b['range']}: {b['hit']*100:.1f}% hit / n={b['n']}")
    ...
    if s.get("recent"):
        ...
        for row in s["recent"]:
            try:
                side, stake, entry_price, shares, edge, confidence, row_status, pnl, ev, result, created_at, market_slug = row
                pnl_txt = f"${float(pnl or 0):.2f}" if row_status == "closed" else "open"
                result_txt = f" | {result}" if result else ""
                lines.append(
                    f"• {str(market_slug or '').replace('btc-updown-15m-', '')} | {side} ${float(stake):.2f} @ {float(entry_price):.3f} "
                    f"| EV ${float(ev or 0):.2f} | {pnl_txt}{result_txt}"
                )
            except Exception:
                continue
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data in ("home", "refresh"):
            ...
            await analytics_cmd(update, context)

        elif data == "strategy_stats":
            await strategy_stats_cmd(update, context)

        elif data == "scalp_stats":
            await scalp_stats_cmd(update, context)
        ...
        elif data == "paper_real_on":
            set_real_odds_only(user_id, True)
            await safe_send(update, "✅ Real-odds-only mode ON.", reply_markup=paper_auto_menu())

        elif data == "paper_real_off":
            set_real_odds_only(user_id, False)
            await safe_send(update, "⚠️ Fallback test mode ON. Real odds are preferred, but fallback_50 paper tests are allowed.", reply_markup=paper_auto_menu())

        elif data.startswith("mode_"):
            mode = data.split("_", 1)[1]
            set_strategy_mode(user_id, mode)
            await safe_send(update, f"✅ Strategy mode set to {mode}.", reply_markup=paper_auto_menu())
...
async def mode_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if mode not in ("hybrid", "scalp", "resolution"):
            await safe_send(update, "Use: /mode hybrid OR /mode scalp OR /mode resolution", reply_markup=paper_auto_menu())
            return
        set_strategy_mode(user_id, mode)
    await safe_send(update, f"Strategy mode: <b>{get_strategy_mode(user_id)}</b>", reply_markup=paper_auto_menu())



async def strategy_stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    touch_user(update)
    await safe_send(update, strategy_breakdown_text(update.effective_user.id), reply_markup=paper_auto_menu())

async def scalp_stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    touch_user(update)
    await safe_send(update, f"<b>⚡ SCALP STATS</b>\n<code>short-term odds movement engine</code>\n\n{scalp_analytics(update.effective_user.id)}", reply_markup=paper_auto_menu())


async def paper_resolve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    touch_user(update)
    user_id = update.effective_user.id
    market, model = await get_btc_context()

    due = due_open_market_slugs(user_id)
    if not due:
        await safe_send(update, "No due open paper trade found yet. It will resolve after market expiry + safety delay.", reply_markup=paper_auto_menu())
        return

    official = {}
    for slug in due:
        official[slug] = await fetch_market_resolution(slug)

    resolved = resolve_open_trades(user_id, model["price"], official)
    if not resolved:
        first = official.get(due[0], {})
        await safe_send(
            update,
            (
                f"⏳ Still waiting for official Polymarket result\n"
                f"Market: <code>{due[0]}</code>\n"
                f"Source: <code>{first.get('source','unknown')}</code>\n"
                f"Debug: <code>{str(first.get('debug',''))[:500]}</code>"
            ),
    ...
    for item in resolved:
        lines.append(
            f"{item['market_slug']} | {item['side']} -> {item['result']} | "
            f"PnL ${item['pnl']:.2f} | source {item.get('resolution_source','')}"
        )
...
async def analytics_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    touch_user(update)
    await safe_send(update, analytics_text(update.effective_user.id), reply_markup=main_menu(is_admin_user(update.effective_user.id)))
    await safe_send(update, strategy_breakdown_text(update.effective_user.id), reply_markup=paper_auto_menu())


async def analytics_export_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    touch_user(update)
    filename, content = export_user_analytics(update.effective_user.id)
    bio = io.BytesIO(content)
    bio.name = filename
    await context.bot.send_document(
        chat_id=update.effective_chat.id,
        document=InputFile(bio, filename=filename),
        caption="📦 Analytics export JSON. Upload this later with /analytics_import."
    )


async def analytics_import_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    touch_user(update)
    set_user_setting(update.effective_user.id, "awaiting_analytics_import", "1")
    await safe_send(update, "Send/upload your polyscalp_analytics JSON file now.")


async def document_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    touch_user(update)
    user_id = update.effective_user.id
    if get_user_setting(user_id, "awaiting_analytics_import", "0") != "1":
        await safe_send(update, "File received. Use /analytics_import first if you want to import analytics.")
        return

    doc = update.message.document
    if not doc:
        await safe_send(update, "No document found.")
        return

    f = await context.bot.get_file(doc.file_id)
    raw = await f.download_as_bytearray()
    try:
        imported = import_user_analytics(user_id, bytes(raw))
        set_user_setting(user_id, "awaiting_analytics_import", "0")
        await safe_send(update, f"✅ Analytics imported: {imported}")
    except Exception as e:
        await safe_send(update, f"Import failed: {e}")
...
def register_handlers(app):
    ...
    app.add_handler(CommandHandler("analytics_export", analytics_export_cmd))
    app.add_handler(CommandHandler("analytics_import", analytics_import_cmd))
    ...
    app.add_handler(CommandHandler("strategy_stats", strategy_stats_cmd))
    app.add_handler(CommandHandler("paper_resolve", paper_resolve_cmd))
