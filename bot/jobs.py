...
async def paper_auto_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for user_row in users:
            ...
            # Fast scalp exits use live odds, not final settlement.
            scalp_closed = resolve_scalp_trades(user_id, market, model)
            for item in scalp_closed:
                emoji = "✅" if item["pnl"] >= 0 else "❌"
                await context.bot.send_message(
                    chat_id=int(chat_id),
                    text=(
                        f"{emoji} <b>Scalp paper trade closed</b>\n"
                        f"Market: <code>{item.get('market_slug','')}</code>\n"
                        f"Side: {item['side']} | Exit: <b>{item['exit_reason']}</b>\n"
                        f"Entry: {item['entry_price']:.3f} → Exit: {item['exit_price']:.3f}\n"
                        f"Stake: ${item['stake']:.2f} | Payout: ${item['payout']:.2f}\n"
                        f"PnL: <b>${item['pnl']:.2f}</b>\n"
                        f"Hold: {item['hold_seconds']:.0f}s | MFE: {item['mfe']*100:.1f}c | MAE: {item['mae']*100:.1f}c"
                    ),
                    parse_mode="HTML",
                    disable_web_page_preview=True,
                )
            ...
            opened = open_scalp_or_resolution_trade(user_id, model)
            if opened.get("opened"):
                await context.bot.send_message(
                    chat_id=int(chat_id),
                    text=(
                        f"🧪 <b>Auto paper trade opened</b>\n"
                        f"Mode: <b>{opened.get('trade_mode','resolution')}</b>\n"
                        f"Strategy setting: <code>{get_strategy_mode(user_id)}</code>\n"
...
async def market_tick_job(context: ContextTypes.DEFAULT_TYPE):
    """Near-real-time market refresh for paper/scalp logic. Polls fast; CLOB WS can replace this later."""
    try:
        cache.delete("btc_bundle")
    except Exception:
        pass
    try:
        await get_btc_bundle()
    except Exception as e:
        logging.debug(f"market_tick_job skipped: {e}")
