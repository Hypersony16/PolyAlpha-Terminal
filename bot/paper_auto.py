...
def get_entry_price(model: Dict[str, Any]) -> float:
    """
    Polymarket binary contract entry price.
    If bot chooses UP, buy UP token at market_up.
    If bot chooses DOWN, buy DOWN token at market_down.
    Fallback is 0.50.
    """
    side = str(model.get("signal", "")).upper()

    if side == "UP":
        price = float(model.get("market_up", model.get("market_prob", 0.5)) or 0.5)
    else:
        price = float(model.get("market_down", model.get("market_prob", 0.5)) or 0.5)

    # Avoid impossible/unsafe prices.
...
def calc_stake(balance: float, model: Dict[str, Any]) -> float:
    ...
    return round(max(MIN_POSITION_USD, min(MAX_POSITION_USD, stake, balance)), 2)
...
def open_auto_trade(user_id: int, model: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """, (
        ...
        float(slippage),
        float(ev),
        f"phase={model['window']['phase']}; binary_payout=1_if_correct",
    ))
    conn.commit()
    conn.close()
