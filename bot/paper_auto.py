...
def open_auto_trade(user_id: int, model: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """, (
        ...
        side,
        float(model["price"]),
        float(model.get("target_price", model.get("open", model["price"]))),
        float(entry_price),
        float(shares),
...
def _log_calibration(user_id: int, row: dict):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
...
def resolve_open_trades(user_id: int, current_price: float) -> List[Dict[str, Any]]:
    ensure_paper_auto_tables()

    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        SELECT id, created_at, window_start, side, entry_btc, open_price,
               entry_price, shares, stake_usd, fee_usd, slippage_usd,
    ...
    for row in rows:
        (
            trade_id, created_at, window_start, side, entry_btc, open_price,
            entry_price, shares, stake, fee, slippage,
            model_prob, market_prob, edge, confidence, note
        ) = row
        ...
        if age < RESOLVE_AFTER_SECONDS:
            continue

        target = float(open_price)
        ...
        balance = get_balance(user_id) + payout
...
def paper_auto_summary(user_id: int) -> Dict[str, Any]:
    ...
    cur.execute("""
        SELECT side, stake_usd, entry_price, shares, edge, confidence, status,
               pnl_usd, ev_usd, result, created_at
        FROM paper_auto_trades
        WHERE user_id = ?
        ORDER BY id DESC
        LIMIT 5
