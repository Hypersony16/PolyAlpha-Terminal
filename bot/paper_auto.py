...
def ensure_paper_auto_tables():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS paper_resolution_waits (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id TEXT NOT NULL,
            created_at TEXT NOT NULL,
            market_slug TEXT,
    ...
    if model:
        try:
            details = (
                ...
                f"entry={float(model.get('entry_price', model.get('market_prob',0)))*100:.1f}c "
                f"trend={model.get('trend_side','?')} vol={model.get('vol_regime','?')}"
            )
        except Exception:
            details = reason
...
def set_resolution_wait_reason(user_id: int, slug: str, reason: str):
    try:
        set_user_setting(user_id, "paper_resolution_wait", f"{slug}: {reason[:220]}")
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO paper_resolution_waits (user_id, created_at, market_slug, reason)
            VALUES (?, ?, ?, ?)
        """, (str(user_id), datetime.utcnow().isoformat(), str(slug or ""), str(reason or "")[:900]))
        conn.commit()
        conn.close()
    except Exception:
        pass
...
def should_enter(user_id: int, model: Dict[str, Any]) -> tuple[bool, str]:
    ...
    # 2) Trade only clean middle of the market window.
    if phase != "Prime":
        return False, f"bad phase: {phase}"

    left = float(model.get("window", {}).get("left_sec", model.get("window", {}).get("seconds_left", 0)) or 0)
    if left < MIN_TIME_LEFT_SECONDS:
        return False, "too close to expiry"

    # 3) Avoid low-price lottery traps and expensive bad R:R entries.
    if entry_price < MIN_ENTRY_PRICE:
        return False, "entry price too cheap / lottery trap"
    if entry_price > MAX_ENTRY_PRICE:
        return False, "entry price too expensive"

    # 4) Require the trade to be broadly aligned with market bias.
    # If the market prices a side below 50%, we do not fight it anymore.
    if market_prob < 0.50:
        return False, "against market bias"

    # 5) Model quality thresholds after probability calibration.
    ...
    # 6) Regime filters: no spike/chop gambling.
    if bool(model.get("spike_flag", False)):
        return False, "last 1m spike too large"
    try:
        if abs(float(model.get("ret_1m_pct", 0))) > 0.18:
            return False, "1m move too large"
    except Exception:
        pass
    if str(model.get("vol_regime", "Normal")) == "High":
        return False, "volatility too high"

    # 7) Trend confirmation. No countertrend bets.
    trend_side = str(model.get("trend_side", "")).upper()
    if trend_side in ("UP", "DOWN") and side in ("UP", "DOWN") and side != trend_side:
        return False, "trend not aligned"

    # 8) Optional market liquidity check when present.
    market = model.get("market") or {}
    try:
...
def resolve_open_trades(user_id: int, current_price: float, official_resolutions: Dict[str, Dict[str, Any]] | None = None) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not resolution or not resolution.get("resolved") or resolution.get("outcome") not in ("UP", "DOWN"):
            # Do not close. Wait for official result. Also save WHY for /paper_auto debug.
            reason = "waiting official outcome"
            if isinstance(resolution, dict):
                reason = f"{resolution.get('source','no_source')} | {resolution.get('debug','no_debug')}"
            set_resolution_wait_reason(user_id, str(market_slug or ""), reason)
            continue
