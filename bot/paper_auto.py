...
def resolve_open_trades(user_id: int, current_price: float) -> List[Dict[str, Any]]:
    """
    Resolve at real 15m window expiry, not 15 minutes after entry.
    This fixes late results when the bot enters with only a few minutes left.
    """
    ensure_paper_auto_tables()
    ...
    for row in rows:
        ...
        if window_end is not None:
            ...
            # Fallback if old DB rows have malformed window_start.
            try:
                created_dt = datetime.fromisoformat(created_at)
            except Exception:
                created_dt = now_dt
            if (now_dt - created_dt).total_seconds() < 900:
                continue
