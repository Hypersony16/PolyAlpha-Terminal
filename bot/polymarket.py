...
async def _market_by_slug(client: httpx.AsyncClient, slug: str) -> Optional[Dict[str, Any]]:
    try:
        r = await client.get(f"{GAMMA_MARKET_SLUG_URL}/{slug}")
        if r.status_code == 200:
            data = r.json()
            if isinstance(data, dict) and data:
                return data
...
async def fetch_market_resolution(slug: str) -> Dict[str, Any]:
    ...
    Resolves from official Gamma fields first, then from settled outcome prices.
    It checks BOTH:
      /events/slug/{slug}
      /markets/slug/{slug}

    Returns resolved=False until the market is actually final.
    """
    slug = str(slug or "").strip()
    if not slug:
        return {"resolved": False, "outcome": None, "source": "missing_slug", "slug": slug, "debug": "missing slug"}
    ...
    async with httpx.AsyncClient(timeout=10, headers=headers) as client:
        ev = await _event_by_slug(client, slug)
        mk_direct = await _market_by_slug(client, slug)

    market_from_event = _market_from_event(ev or {}) if ev else {}
    candidates = []
    if isinstance(ev, dict):
        candidates.append(("event", ev))
    if isinstance(market_from_event, dict) and market_from_event:
        candidates.append(("event_market", market_from_event))
    if isinstance(mk_direct, dict) and mk_direct:
        candidates.append(("market_slug", mk_direct))

    if not candidates:
        return {
            "resolved": False,
            "outcome": None,
            "source": "gamma_missing",
            "slug": slug,
            "debug": "event/market slug endpoints returned no data",
        }

    # 1) explicit official winner fields
    for label, obj in candidates:
        outcome = _extract_official_outcome_from_obj(obj)
        if outcome in ("UP", "DOWN"):
            return {
                "resolved": True,
                "outcome": outcome,
                "source": f"gamma_official_{label}",
                "slug": slug,
                "debug": f"explicit outcome from {label}",
                "question": str(obj.get("question") or obj.get("title") or ""),
            }

    # 2) final/closed status + settled outcome prices
    debug_parts = []
    for label, obj in candidates:
        final = _event_is_final(obj)
        closed_val = obj.get("closed")
        resolved_val = obj.get("resolved")
        active_val = obj.get("active")
        prices = _extract_gamma_prices(obj)
        up = prices.get("up")
        down = prices.get("down")
        debug_parts.append(f"{label}: final={final} closed={closed_val} resolved={resolved_val} active={active_val} up={up} down={down}")

        if final and up is not None and down is not None:
            # Many resolved binary markets become 1/0 or 0/1.
            if up >= 0.98 and down <= 0.02:
                return {
                    "resolved": True,
                    "outcome": "UP",
                    "source": f"gamma_price_settlement_{label}",
                    "slug": slug,
                    "debug": f"settled prices from {label}: up={up}, down={down}",
                    "question": str(obj.get("question") or obj.get("title") or ""),
                }
            if down >= 0.98 and up <= 0.02:
                return {
                    "resolved": True,
                    "outcome": "DOWN",
                    "source": f"gamma_price_settlement_{label}",
    ...
    # 3) closed but no clear winner = still wait, do NOT guess by BTC price
    return {
        "resolved": False,
        "outcome": None,
        "source": "gamma_unresolved",
        "slug": slug,
        "debug": " | ".join(debug_parts)[:900],
    }
