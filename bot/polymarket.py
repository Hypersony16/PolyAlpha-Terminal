...
def _looks_like_btc_15m(text: str) -> bool:
    ...
    if not any(x in t for x in ["15m", "15 min", "15-minute", "15 minute"]):
        return False
    # Must mention direction; avoid false positives from "btc up 15m volume" etc.
    if not any(x in t for x in ["up or down", "up/down", "up-down", "higher or lower"]):
        return False
    return True
...
async def _fill_clob_prices(market: Dict[str, Any]):
    """
    FIX: Always attempt CLOB prices when token IDs exist.
    CLOB provides live BUY prices; Gamma outcomePrices are often stale mid-prices.
    This is called both as primary source (when token IDs are present)
    and as fallback (when Gamma prices are missing).
    """
    up_token = market.get("up_token_id")
    down_token = market.get("down_token_id")

    if not up_token and not down_token:
        return  # No token IDs, cannot query CLOB.
    ...
    # If only one side came back, infer the other.
...
async def discover_btc_15m_market() -> Dict[str, Any]:
    """
    BTC 15m market discovery with real live odds.
    Always attempts CLOB prices (live BUY prices) when token IDs exist.
    Gamma outcomePrices are stale mid-prices and are only used as last resort.
    Returns {} if no market found. Never raises.
    """
    now = time.time()
    ...
    # FIX: ALWAYS try CLOB when token IDs exist.
    # CLOB gives live BUY prices. Gamma prices are stale midpoints.
    # _fill_clob_prices will overwrite Gamma prices if CLOB responds successfully.
    has_tokens = market.get("up_token_id") or market.get("down_token_id")
    if has_tokens:
        await _fill_clob_prices(market)
    elif market.get("up_price") is None or market.get("down_price") is None:
        # No token IDs and missing prices — nothing we can do.
        pass
