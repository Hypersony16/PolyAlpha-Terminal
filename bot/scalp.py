...
SCALP_ENTRY_MAX = 0.55       # scalps should not buy expensive contracts
SCALP_ENTRY_MIN = 0.12
SCALP_MIN_EDGE = 0.12
SCALP_MIN_TIME_LEFT = 240
SCALP_MAX_TIME_LEFT = 820
...
def choose_trade_mode(user_id: int, model: Dict[str, Any]) -> str:
    ...
    left = float(model.get("window", {}).get("seconds_left", 0) or 0)
    ...
    if entry <= 0.50 and edge >= 0.12 and conf == "High":
        return "resolution"
...
def resolve_scalp_trades(user_id: int, market: Dict[str, Any], model: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if hold_seconds >= SCALP_MIN_HOLD_SECONDS and live_price >= target:
            ...
            reason = "time_exit"
        elif float(model.get("window", {}).get("seconds_left", 999) or 999) < 75:
