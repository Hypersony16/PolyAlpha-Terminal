...
SCORE_FLOOR_FOR_CONSENSUS = 50.0   # only wallets >= this contribute to consensus
MIN_CONSENSUS_WALLETS = 3           # minimum smart wallets overlapping a position
MIN_CONSENSUS_VALUE = 1000.0        # min total USD across all wallets in signal
...
class PolyAlphaClient:
    ...
    async def fetch_positions(self, wallet: str, limit: int = 250) -> list[Position]:
        ...
        for p in rows:
            ...
            if title or market:
                out.append(Position(wallet, market, title or market, outcome or "YES", size, value, avg_price, cur, token_id, condition_id))
        return out
