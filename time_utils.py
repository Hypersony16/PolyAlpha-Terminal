...
from bot.alpha_store import save_wallet_score, save_positions, save_consensus, save_discovered_wallet, save_alpha_scan_run, clear_old_consensus
...
def _rank_score_from_leaderboard(lb: LeaderboardWallet, total_limit: int = 100) -> WalletScore:
    """Fast score from leaderboard data.

    The Data API sometimes returns no realized PnL/win-rate fields for some boards.
    In that case we still rank wallets by leaderboard rank, volume and open exposure
    instead of giving every wallet the same useless 31.5 score.
    """
    raw = lb.raw or {}
    ...
    trades = int(_first_metric(raw, ("trades", "tradeCount", "numTrades", "positions", "marketsTraded"), 0.0)) or 0
    winrate = _first_metric(raw, ("winRate", "winrate", "win_rate", "accuracy", "hitRate"), 0.0)
    if 0 < winrate <= 1.0:
        winrate *= 100.0
    # If no trade count is exposed, use a small synthetic count based on rank/volume so it does not display as 500 fake trades.
    if trades <= 0:
        trades = max(1, int(min(250, math.log10(max(10.0, volume + open_value)) * 25)))
    roi = 0.0
    if volume > 0 and pnl:
        roi = pnl / max(1.0, volume) * 100.0
    # Rank quality matters when public detailed pnl fields are missing.
    rank_score = max(0.0, 40.0 * (1.0 - min(max(lb.rank - 1, 0), total_limit) / max(1, total_limit)))
    pnl_score = max(0.0, min(25.0, math.log10(max(1.0, abs(pnl))) * 5.0)) if pnl > 0 else 0.0
    volume_score = min(18.0, math.log10(max(1.0, volume)) * 2.3)
    open_score = min(10.0, math.log10(max(1.0, open_value)) * 1.2)
    wr_score = min(12.0, max(0.0, winrate - 45.0) * 0.45) if winrate else 0.0
    score = max(1.0, min(100.0, rank_score + pnl_score + volume_score + open_score + wr_score))
    if pnl < 0:
        score *= 0.55
    return WalletScore(lb.wallet, round(score, 1), round(roi, 1), round(pnl, 2), round(volume, 2), trades, round(winrate, 1), round(open_value, 2), consistency=round(rank_score,1), recent_score=0.0, drawdown=0.0, label=lb.username or "leaderboard")
...
class PolyAlphaClient:
    ...
    async def fetch_leaderboard(self, category: str = "OVERALL", time_period: str = "MONTH", order_by: str = "PNL", limit: int = 100) -> list[LeaderboardWallet]:
        """Fetch Polymarket's official public trader leaderboard.

        Current official Data API endpoint is /v1/leaderboard and max limit is 50,
        so requests above 50 are paginated with offset. Older endpoints are kept
        only as fallback, never as the first request.
        """
        ...
        async def parse_rows(data: Any, start_rank: int = 1) -> list[LeaderboardWallet]:
            ...
            for idx, r in enumerate(rows, start_rank):
                if not isinstance(r, dict):
                    continue
                wallet = _s(r.get("proxyWallet") or r.get("profileAddress") or r.get("wallet") or r.get("address") or r.get("user") or r.get("funder"))
        ...
        # Official endpoint. Polymarket docs: https://data-api.polymarket.com/v1/leaderboard
        out: list[LeaderboardWallet] = []
        ...
        try:
            ...
            while len(out) < total_limit and offset <= 1000:
                ...
                data = await self._get(f"{DATA_API}/v1/leaderboard", {
                    "category": category,
            ...
            for w in out:
                if w.wallet not in seen:
                    seen.add(w.wallet); deduped.append(w)
            if deduped:
                return deduped[:total_limit]
        except Exception as e:
            last_error = e

        # Legacy fallbacks for older deployments only.
        for url in (f"{DATA_API}/leaderboard", f"{DATA_API}/rankings"):
            try:
                data = await self._get(url, {"category": category, "timePeriod": time_period, "orderBy": order_by, "limit": min(total_limit, 50)})
...
class SmartMoneyEngine:
    ...
    async def score_wallet(self, wallet: str) -> WalletScore:
        ...
        trades = len(activity)
        volume = sum(_f(x.get("size") or x.get("amount") or x.get("usdcSize") or x.get("value") or x.get("volume")) for x in activity)
    ...
    async def discover_from_leaderboards(self, category: str = "OVERALL", time_period: str = "MONTH", order_by: str = "PNL", limit: int = 100, score_top: int = 50) -> dict[str, Any]:
        ...
        try:
            ...
            # Fast leaderboard-based scores first. This avoids the slow/fake 0% ROI rescans.
            lb_scores = sorted([_rank_score_from_leaderboard(w, total_limit=max(50, limit)) for w in wallets_found[:max(1, score_top)]], key=lambda x: x.score, reverse=True)
            for sc in lb_scores:
                save_wallet_score(sc)

            # Fetch positions only for the best wallets. This drastically reduces latency.
            best_wallets = [s.wallet for s in lb_scores[:min(30, len(lb_scores))]]
            pos_batches = await asyncio.gather(*(self.client.fetch_positions(w, 80) for w in best_wallets), return_exceptions=True) if best_wallets else []
            for w, rows in zip(best_wallets, pos_batches):
                if isinstance(rows, list):
                    save_positions(w, rows)

            # Build consensus from cached/highest-score wallets only.
            consensus = await self.consensus(best_wallets, min_wallets=2, top_n=20) if best_wallets else []
            clear_old_consensus()
            top = lb_scores[0] if lb_scores else None
            save_alpha_scan_run("leaderboard", category, time_period, order_by, len(wallets_found), len(wallets_found), len(lb_scores), top.wallet if top else "", top.score if top else 0, "ok", "")
            return {"status":"ok", "wallets_found": len(wallets_found), "wallets_added": len(wallets_found), "wallets_scored": len(lb_scores), "top_wallet": top.wallet if top else "", "top_score": top.score if top else 0, "consensus": len(consensus), "scores": lb_scores, "signals": consensus}
        except Exception as e:
    ...
    async def discover_multi_leaderboards(self, limit_per_board: int = 75, score_top: int = 75) -> dict[str, Any]:
        ...
        scores = sorted([_rank_score_from_leaderboard(w, total_limit=max(50, len(candidates))) for w in candidates[:max(1, score_top)]], key=lambda x: x.score, reverse=True)
        for sc in scores:
            save_wallet_score(sc)
        best_wallets = [s.wallet for s in scores[:min(35, len(scores))]]
        pos_batches = await asyncio.gather(*(self.client.fetch_positions(w, 80) for w in best_wallets), return_exceptions=True) if best_wallets else []
        for w, rows in zip(best_wallets, pos_batches):
            if isinstance(rows, list):
                save_positions(w, rows)
        signals = await self.consensus(best_wallets, min_wallets=2, top_n=20) if best_wallets else []
        clear_old_consensus()
